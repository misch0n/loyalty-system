...
- **Shared components:** `packages/web/src/ui/components/<Name>/` — one folder per component (`Name.tsx` + `Name.css` + `Name.test.tsx`). Components: Logo, Heading, Button (+WalletButton), Field, CupStamps, LoyaltyCard, Qr, Overlay, Toast, PinPad, Slider, Sheet (+MenuRow +RecoveryLine), ContextBanner. No business logic in components.
- **Structure:** `packages/web/src/ui/app/` — `LogoGestures` (logo tap/long-press handlers; replaces the old Shell), `AuthContext`, `EntryResolver`, `routes.ts`, `session.ts`. `packages/web/src/ui/screens/<area>/<Screen>/` — folder-per-screen (`Screen.tsx` + `Screen.css` + `Screen.test.tsx`); screen-scoped parts live in `_parts/` beside the screen. `packages/web/src/ui/common/` — ServicesContext, PairingContext, QrDisplay, QrScanner, PrivacyNotice, PairDevices.
