...
- **Navigation:** no home dashboard. Recognized customer → `/card/:token` (hub). Unrecognized → `/welcome`. Entry resolved by `EntryResolver` at `/`. Logo gestures: tap **left half** → home (`/` → entry resolver); tap **right half** → Prototype panel (prototype build only, gated on `isPrototype` — adapter selection, NOT `import.meta.env.PROD`, since the GitHub Pages demo is itself a production build); long-press (≥600ms) → staff/admin sign-in. Keyboard: Enter-tap → home, the "Staff sign-in" link → sign-in.
