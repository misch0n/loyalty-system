...
- **Reward threshold: 8** (`pointsPerReward: 8` in seed) — eight coffees earn the reward. The card still **displays a 10-cup grid** (`CupStamps showcase`): a welcome "sticker" cup (on the house), the 8 earnable cups, and a final **FREE** reward cup. There is no "Gold" tier badge (removed).
