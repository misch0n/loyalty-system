...
- **Navigation:** no home dashboard. Recognized customer → `/card/:token` (hub). Unrecognized → `/welcome`. Entry resolved by `EntryResolver` at `/`. Logo gestures (`LogoGestures`): tap **left half** → home; tap **right half** → Prototype panel (gated on `isPrototype` — adapter selection, NOT `import.meta.env.PROD`); long-press (≥600ms) → staff/admin sign-in. Visually-hidden keyboard path to sign-in. **No global "Staff sign-in" subtitle** in the layout.
