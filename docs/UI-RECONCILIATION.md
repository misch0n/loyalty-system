...
| # | Backend truth | UI today | Status |
|---|---|---|---|
| A1 | **Done — UI-0** (the sheet, the past-exports list, `exportActivity`, `ExportRecord`, `parseExportRecord` and their tests are deleted). **No cross-account activity endpoint exists.** `GET /audit` replaces the request's actor filter with the session's own at every tier; there is no export route and the database refuses an `audit.export` row (STATUS divergence **r**, §4-F). | The admin **Export activity** sheet, the past-exports list, and `AuditService.exportActivity` / `parseExportRecord` all still exist. | **Settled** (triage §1, BE-A-12) |
| A2 | **Done — UI-0** (the four tiles, the `Stat` tile component and `StatDetail` are deleted; `StatWide`, which the Configure panel uses, stays. `@cafe/shared/domain/insights.ts` stays too — the server's activity route reads it). **No stats surface.** The data is still collected; `GET /stats/active-customers` and the ranged `GET /transactions` exist, but the triage dropped the presentation. | Four stat tiles and the `StatDetail` breakdown popover. | **Settled** (triage §1, FE-A-02/03) |
...
| # | Backend truth | UI today | Status |
|---|---|---|---|
| P1 | `src/adapters/sync/` is deleted — there is no device pairing, and a server coordinates state centrally. | `ProtoPanel`'s "Scan to pair", `PairDevices`, `PairingContext`, the `/pair` route, `DevTrigger`. | **Done — UI-0.** All of it deleted, plus `ui/common/storageSnapshot.ts` (the pairing push/pop had no other caller) and `e2e/prototype.e2e.ts`. One consequence to carry into **UI-5**: `Card` and `Panel` keyed their refetch on `usePairing().dataVersion` and now load once per visit |
| P2 | The `WalletProvider` and `Transport` ports are deleted (triage §1). | The wallet button (`FE-C-09`), `WalletButton`, the `'w'` scan source in the commit payload. | **Done — UI-0.** The button, `WalletButton` + its CSS/tests, `EnlargedQr`'s pass resolution (and its now-pointless `customerId` prop) and the Scan's `pushWallet` are gone. **Decided: the `source: 'a' \| 'w'` enum stays** — the ledger records it harmlessly and shrinking it would edit the shared port and the server's schema for no UI gain |
...
| X6 | **Cross-device liveness is `GET /events`, an SSE stream carrying a signal.** `event: changed` with `{scope, id, reason}` — `reason` is `commit` / `ledger` / `card` / `deleted` / `activity` — and the screen re-reads through the routes it already had. **The subjects come from the session**: a customer's device hears about its own card, a till about its own actor. Anonymous and idle-locked callers are refused; 3 streams per session; the stream ends on sign-out, "sign out all devices", the account being disabled or deleted, a rebind, and a card deletion (STATUS divergence **ae**). | `usePairing().dataVersion` is a counter bumped by `PairingContext` over the deleted PeerJS layer; `Card` and `Panel` refetch on it. Nothing opens an `EventSource`. | **Settled** on the backend shape — the client subscriber is UI-pass work. Two things it has to decide, and neither is a backend question: **(a)** `PairingContext` is deleted (row P1), so what replaces it as the thing that owns the connection and exposes a `dataVersion`; **(b)** `reason: 'deleted'` and `reason: 'card'` are **new information the prototype never had** — a card that vanished under the customer, and a QR that just went stale — and what a screen does with each is a design call. |
| X7 | **Two screen-visible things have no push behind them, deliberately.** A `PATCH /config` (the reward threshold every card renders) and staff-account changes publish nothing: the scope is per customer and per till (BACKEND-PLAN §3-C-15), an admin's own device made the change, and another till picks it up on its next read. | The prototype's pairing `dataVersion` was global — *any* shared-data write refreshed *every* paired screen, config included. | **Confirm** — narrower than the prototype on purpose, and cheap to widen (a `program` scope every session subscribes to) if a stale threshold on an open card turns out to matter. Worth an answer before the UI pass builds around the narrower guarantee. |
| X8 | **The SPA is excluded from both gates, in two named places, and the UI pass is what closes them.** CI's `web` job is `continue-on-error` and reports the red rather than judging it (`.github/workflows/ci.yml`); `compose.yml`'s `web` service sits behind a `web` profile so `docker compose up` does not stall on a package that does not compile; the `web` image is not built in CI at all. | `@cafe/web`: 9 failing test files and a failing `tsc -b`, every one of them traceable to a Phase 6 deletion. | **Settled** — mechanical, and recorded so "the UI pass is finished" has a definition: `continue-on-error` comes off the `web` job, `profiles: ['web']` comes off the service, and the image build joins the `bundle` job. Also a tripwire: CI reports the shape of the red every commit, so a **tenth** failing file is something new rather than something Phase 6 caused. |
...
- **2026-09-16, Phase 9** — CI. It produced **no new conflicts**: the phase adds a workflow and two
  ops scripts and changes no product behaviour. One row anyway — **X8**, which writes down the two
  places the SPA is currently excluded from a gate (CI's `web` job, the `web` Compose profile) so
  that "the UI pass is finished" has a mechanical definition rather than a feeling. It also makes
  the register's other rows cheaper to trust: CI now reports the *shape* of the SPA's red on every
  commit, so a tenth failing test file would be noticed immediately rather than discovered by the
  UI pass. **This is the last backend phase.** Every row below is now the UI pass's to answer, and
  the ones marked **Open** or **Confirm** still need the maintainer.
- **2026-09-16, UI-0** — the first UI-pass phase, and pure deletion. **P1, P2, P4, P5, A1 and A2
  are Done**; each row above says what went and what the phase decided where it had a choice. Two
  decisions are worth confirming because they are the kind that are cheap now and expensive later:
  the **`source: 'a' | 'w'` enum stays** (P2 — the alternative edits the shared port and a server
  schema for nothing a user sees), and `domain/insights.ts` stays in `@cafe/shared` because the
  server reads it even though no screen does (A2). **One new fact for UI-5, not a new conflict:**
  deleting `PairingContext` took `Card`'s and `Panel`'s live refetch with it, so both now load once
  per visit — row **X6** already owns the replacement, and this is the concrete gap it closes. Also
  a correction to the numbers every later phase is measured against: the SPA has **47** test files,
  not the 53 UI-PLAN opened with (Phase 10 moved six domain suites into `@cafe/shared`); the
  baseline was 9 of 47 failing, and after UI-0 it is **6 of 44**, with the TypeScript error count
  **39 → 14**. Nothing else here has been confirmed yet.
- **2026-09-16, Phase 7** — the SSE channel. **X6** is the replacement for the pairing layer's
  `dataVersion` and is **Settled** on the backend's side: the shape, the subjects and the
  lifecycle are built and tested, and what is left is a client subscriber the UI pass writes. It
  carries two design decisions the prototype never faced, because `reason: 'deleted'` and
  `reason: 'card'` are information the pairing layer could not deliver. **X7** is the one
  genuinely new question the phase raises — the channel is deliberately narrower than the
  prototype's global `dataVersion` (no push for a config or staff-account change), which is cheap
  to widen and much harder to narrow once screens assume it. Still nothing confirmed.
