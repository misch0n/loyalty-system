 ...
 */
import type { AuditAction, AuditLogEntry } from './models.js';
...
export function buildInsight(
  ...
  const windowStart = slots[0]?.start ?? now; // every branch of `slotsFor` pushes at least five
  ...
  if (unique) {
    ...
    buckets.forEach((b, i) => (b.value = seen[i]?.size ?? 0));
  ...
  for (const e of matched) {
    ...
    // `findIndex` returns -1 for an entry in no slot, and `buckets[-1]` is undefined.
    const bucket = buckets[slots.findIndex((s) => time(e) >= s.start && time(e) < s.end)];
